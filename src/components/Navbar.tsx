...
import { Sprout, ShoppingBag, Search, MessageSquareHeart, PhoneCall, ShieldCheck, Menu, X, Leaf } from 'lucide-react';

interface NavbarProps {
  currentTab: 'home' | 'create' | 'orders' | 'feedback' | 'admin';
  onNavigate: (tab: 'home' | 'create' | 'orders' | 'feedback' | 'admin') => void;
  onOpenContact: () => void;
}

export const Navbar: React.FC<NavbarProps> = ({ currentTab, onNavigate, onOpenContact }) => {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const handleNavClick = (tab: 'home' | 'create' | 'orders' | 'feedback' | 'admin') => {
    onNavigate(tab);
    ...
    <header className="sticky top-0 z-40 bg-[#FBF9F5]/90 backdrop-blur-md border-b border-[#E8E2D9] transition-all">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          >
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-[#2D6A4F] to-[#1B4332] flex items-center justify-center text-white shadow-sm shadow-[#2D6A4F]/20 group-hover:scale-105 transition-transform duration-200">
          ...
          <nav className="hidden md:flex items-center gap-1 lg:gap-2">
            <button
              id="nav-home-btn"
              onClick={() => handleNavClick('home')}
              className={`px-3.5 py-2 rounded-xl text-sm font-semibold transition-colors duration-150 ${
                currentTab === 'home'
                  ? 'bg-[#EAF5EE] text-[#1B4332] shadow-xs'
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#FAF7F2] border-b border-[#E8E2D9] px-4 pt-3 pb-5 space-y-2 animate-fadeIn">
          <button
            onClick={() => handleNavClick('home')}
